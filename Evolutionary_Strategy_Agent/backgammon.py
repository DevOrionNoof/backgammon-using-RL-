...
class backgammon:
  ...
  def check_winner(self):
    self.winner = None
    if self.board[NUM_POINTS + NUM_POINTS] == 15:
      self.winner = "WHITE"
    elif self.board[NUM_POINTS + NUM_POINTS + 1] == 15:
      self.winner = "BLACK"
  ...
  def move_check(self,point, roll, board):
    player = board.copy()
    if player[NUM_POINTS*2+2] == 1:
      min = 0
      max = NUM_POINTS
      new_move = player[min:max]
      off_idx = NUM_POINTS + NUM_POINTS
    else:
      min = NUM_POINTS
      max = NUM_POINTS + NUM_POINTS
      new_move = player[min:max]
      off_idx = NUM_POINTS + NUM_POINTS + 1
    ...
    if point < highest_point:
      return None
    if (point == highest_point) and (point+roll >= NUM_POINTS):
      new_move[point] -= 1
      player[off_idx] += 1
    elif (point+roll == NUM_POINTS):
      new_move[point] -= 1
      player[off_idx] += 1
    elif (point+roll < NUM_POINTS):
      new_move[point] -= 1
